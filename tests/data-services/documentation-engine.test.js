...
// Mock dependencies
jest.mock('../../bmad-method/tools/lib/security-logger', () => ({
    securityLogger: {
        logDataOperation: jest.fn()
    }
}));

jest.mock('../../bmad-method/tools/lib/yaml-utils', () => ({
    loadYaml: jest.fn()
}));
...
describe('DocumentationEngine', () => {
    ...
    beforeEach(() => {
        ...
        // Mock yaml-utils
        mockYamlUtils = require('../../bmad-method/tools/lib/yaml-utils');
    ...
    describe('Configuration Reference Generation', () => {
        test('should generate configuration reference', async () => {
            const testConfig = {
                database: {
                    host: 'localhost',
                    port: 5432,
            ...
            await createTestFile(configFile, JSON.stringify(testConfig));
            
            const sourceInfo = { configs: [configFile] };
            const result = await engine.generateConfigurationReference(sourceInfo);
            
            expect(result.success).toBe(true);
            expect(result.data.configurations).toHaveLength(1);
            expect(result.data.configurations[0].config).toEqual(testConfig);
            expect(Object.keys(result.data.configurations[0].schema)).toContain('database.host');
            expect(Object.keys(result.data.configurations[0].schema)).toContain('api.timeout');
            expect(result.data.summary.total_configs).toBe(1);
        });
    });
    ...
    describe('Integration with BMad Patterns', () => {
        ...
        test('should use BMad YAML utilities for configuration loading', async () => {
            ...
            expect(mockYamlUtils.loadYaml).toHaveBeenCalledWith(
                expect.stringContaining('bmad-method/config/feature-flags.yaml')
            );
        });
